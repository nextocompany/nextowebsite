...
must_haves:
  truths:
    ...
    - "Lighthouse mobile performance score is 90 or above, verified by CLI audit"
...
<task type="auto">
  ...
  <action>
    ...
    7. **Lighthouse mobile audit** — Serve the build locally and run Lighthouse:
       ```
       npx serve out -l 3456 &
       npx lighthouse http://localhost:3456 --only-categories=performance --form-factor=mobile --chrome-flags="--headless" --output=json --output-path=./lighthouse-report.json
       ```
       Parse the JSON for `categories.performance.score` — must be >= 0.90 (i.e. 90+). If below 90, investigate and fix performance issues before deploying. Clean up: kill the serve process and delete the report file.

    If any check fails, fix the issue before proceeding. This is the gate before deployment.
  </action>
  <verify>
    All 7 checks above pass. Build output is clean and complete. Lighthouse mobile performance score >= 90.
  </verify>
  <done>
    Build succeeds. All expected files present in out/. SEO tags correct. Page weight under 1MB. Lighthouse mobile score 90+. Sitemap and robots.txt valid.
  </done>
</task>
